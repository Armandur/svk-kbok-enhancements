...
Hur tillägget är byggt och varför lösningarna ser ut som de gör, samt de
fällor i Kbok som kartlades under arbetet med användarmanualen.

[README.md](README.md) beskriver vad tillägget gör och hur man installerar
det. Den här filen är för den som vill veta varför.
...
minuter efter en push. Uppmätt: push 20:53, ny version synlig 20:58. Det
gäller både Tampermonkeys kontroll och tilläggets egen, så en nyss utgiven
version kan se ut att inte finnas.
...
## Tre mindre lösningar

**Tabb förbi datumväljaren.** Kalender- och klockknappen får
`tabindex="-1"`, så tabb går från datumfältet vidare i formuläret i stället
för in i väljaren.

**Markerbart personnummer.** PERSNR-cellen görs markerbar så numret går att
dra över och kopiera. Griden fångar annars klicket och öppnar posten.

**D för dagens datum.** Formatet läses ur fältets placeholder - dödsdatum
vill ha ÅÅÅÅMMDD, övriga ÅÅÅÅ-MM-DD.
...
`verktyg/skarmdumpar.py` tar om bilderna i `skarmdumpar/`. Den loggar in i
Utbildningsmiljön, injicerar skriptet med Playwrights `add_init_script`
(motsvarar `@run-at document-start`) och fotograferar panelen,
träfflistan och visningsrutan.

Två saker måste stämma: registreringen av init-skriptet ska ske före
inloggningens första navigering, annars hinner appen starta först, och
webbläsaren ska startas med `channel="chromium"` - standard-headless
